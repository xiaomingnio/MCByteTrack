...
yolox::yolox()
{
    string model_path = "./yolox_tiny.onnx";
...
int main()
{
    std::string videopath = "./test_13.mp4";
    ...
    VideoWriter writer("demo1.avi", cv::VideoWriter::fourcc('M', 'J', 'P', 'G'), fps, Size(img_w, img_h));
	...
	for (;;)
    {
        ...
        std::map<int, vector<Object>>  objects_all;
        for (int i=0; i< objects.size(); i++){
//            std::cout << objects[i].label << std::endl;
            if(objects_all.count(objects[i].label) == 0){
                vector<Object> tmp{objects[i]};
                objects_all.insert(make_pair(objects[i].label, tmp));
            }
            else{
                objects_all[objects[i].label].push_back(objects[i]);
            }

        }
        std::map<int, vector<STrack>> output_stracks = tracker.update(objects_all);
        auto end = chrono::system_clock::now();
        total_ms = total_ms + chrono::duration_cast<chrono::microseconds>(end - start).count();
        for (int cls = 0; cls < 80; cls++)
		{
		    for (auto out : output_stracks[cls]){
                vector<float> tlwh = out.tlwh;
                bool vertical = tlwh[2] / tlwh[3] > 1.6;
                if (tlwh[2] * tlwh[3] > 20 && !vertical)
                {
                    Scalar s = tracker.get_color(out.track_id);
                    string ss = "cls: " + to_string(cls) + " id: " + to_string(out.track_id);
                    putText(img, ss, Point(tlwh[0], tlwh[1] - 5),
